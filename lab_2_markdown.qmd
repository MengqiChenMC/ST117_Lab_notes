---
title: "ST117 Lab 2"
subtitle: "dataframe, random numbers, and functions"
author: "Mengqi Chen"
date: last-modified
format: html
editor: visual
...
Recall that last time we generated a random dataframe of columns named "id" and "score" first.
...
In this case, you would run into problems if you simply taking the first column of `data_read`:
...
The `apply` family of functions in R is used to apply a function over the margins of an array or matrix, and it's particularly useful for dataframes. For example, `apply` can be used to apply a function to each column or row of a dataframe.
...
This will calculate the mean of each column in the dataframe `df`.
...
The following is the Lab 2 work sheet designed by **Yujing Lu**, the module tutor.
...
## 3.1 Random numbers and dataframe
...
We aim to record names, assignment marks, exam marks of 30 students from course 1 in a table. Using the functions above, first generate these items randomly, then combine them into a data frame with three columns: "Names", "Assignment", and "Exam".
...
\(ii\) Can you find the student with highest "FinalMark"?
...
```{r include=FALSE}
course1 %>% arrange(desc(FinalMark)) %>% head()
```

\(iii\) Next, calculate the means of "Assignment","Exam" and "FinalMarks".
...
\(iv\) Then, randomly pick 10 of the students who have also chosen course 2, and add a column to indicate their choice (0 for yes,1 for no).
...
\(v\) Finally, find the students with marks above 85 and haven't chosen course 2 by extracting values from "names" column. You can also use `apply()` function for this purpose. Verify if these two methods give the same results.
...
Simulate 10000 samples from a uniform distribution $X\sim U[0,1]$ and create a histogram of the generated sequence. Overlay the histogram on the probability density function - does the histogram reflect the density well?
...
title<-sprintf("Histogram of %d simulations from X~U(%d,%d)", 10000, 0, 1)
    hist(sample, main = title, xlim = c(-0.5, 1.5), xlab = "", prob = TRUE,col="lightblue", ylab="f (x)")
...
This is a more complicated simulation game and is by no means compulsory - try it if you are interested!
:::
